...
var Dom = class {
   ...
   */
  append(...children) {
    this.#children.push(...import_genstack3.GenStack.from(children).filterNullUndefined());
    return this;
  }
  ...
  toStringTagOpener() {
    const tagOpener = [
      this.#tagname,
      ...import_genstack3.GenStack.from(this.#attributes.entries()).map(([k, v]) => {
    ...
    if (this.#classList.size) {
      const classes = import_genstack3.GenStack.from(this.#classList).map((c) => typeof c === "function" ? c() : c).filterNullUndefined().map((c) => c.trim()).filter((c) => c.length > 0).toArray();
      if (classes.length) {
...
var Style = class extends Dom {
   ...
   */
  get length() {
    return import_genstack4.GenStack.from(this.#stylesheets.values()).filter((s) => s.length > 0).toArray().length;
  }
  /**
